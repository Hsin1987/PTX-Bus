...
def create_date_header():
    #now = datetime.now()
    #tamp = mktime(now.timetuple())
    #time = format_date_time(stamp)
    return datetime.now().strftime("%a, %d %b %Y %H:%M:%S GMT")#format_date_time(stamp)
...
def generate_request_headers(key_id, secret, url, data=None, content_type=None):
    ...
    signature_hash = sha1_hash_base64(signature_headers, secret)
    # Format the authorization header
    auth_header = auth_header_template.format(key_id, algorithm, headers, signature_hash.decode("utf-8"))

    if request_method == 'GET':
        request_headers = {
            'Authorization': auth_header,
            'Date': signature_headers
            }
    else:
        request_headers = {
            'Authorization': auth_header,
